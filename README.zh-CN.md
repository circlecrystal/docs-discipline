...
- 🛰️ **漂移雷达。** 揪出失效链接、过期时间戳、没人引用的孤儿文档，同一个“事实”在五个地方各说一遍，还有 B 层“当前状态”文档被逐 session 叙述悄悄撑大——这些正是文档开始撒谎的根源。
...
> *提示：* docs-discipline 也在申请进入 Anthropic 的社区 marketplace。一旦上架，你就能改用 `/plugin marketplace add anthropics/claude-plugins-community` + `/plugin install docs-discipline@claude-community`。在那之前，上面的直接安装就是正路。
...
两条斜杠命令加一个脚本——没有任何东西会自行运行：

| 组件 | 是什么 |
| --- | --- |
| `/docs-discipline:codify` | session 收尾仪式：自举 → 沉淀 → 体检 → 可选交接 |
| `/docs-discipline:review` | 独立的文档健康体检（漂移 + SSOT） |
| `scripts/drift-check.sh` | 一个只读的 Bash 扫描器，首次运行时拷进你的仓库，承载漂移启发式（也可在 CI/cron 里直接调） |
| `CLAUDE.md` 桩 | 一段最小的习惯锚点 + A/B 骨架，首次运行时创建或追加 |

**没有 agent、没有 hook、没有 MCP server、没有 LSP server。** 没有任何东西挂进你的 session 循环，所以每轮的上下文开销基本为零——命令只在*你*主动调用时才跑。

## 环境要求

- **Claude Code**，且支持插件（有 `/plugin` 命令）。
- **Bash**，用于 `drift-check.sh`（macOS/Linux；BSD 与 GNU 的 `date` 都兼容）。agent 驱动的阶段无需它——只有可脚本化的漂移扫描需要 shell。
- **Git** 可选：`drift-check.sh` 有 git 时用它（尊重 `.gitignore`、读取提交日期），没有时退回到普通文件遍历。

## 安全与权限——它碰什么

docs-discipline 刻意保持低信任。它做什么、不做什么，一清二楚：

- **读取** 你仓库的 markdown 和 `CLAUDE.md`，外加经由 `drift-check.sh` 的只读 git 元数据（`git ls-files`、`git log`）。
- **只写你批准的东西。** 首次运行时，它可能创建 `CLAUDE.md`（或往已有的文件追加一段声明块），并把 `drift-check.sh` 拷进 `scripts/`——而且它总会打印一张回执，如实说明创建/追加/拷贝/跳过了什么。其余所有写入（A/B 文档改动、交接文档）都是*先以清单形式提议*，经你点头才落盘。
- **绝不静默写入，绝不自动修。** 体检阶段只做分诊——摆出候选项，由你定夺。
- **无网络、无遥测、无外部服务。** `drift-check.sh` 就是个纯 Bash 扫描器，读文件、给出退出码 + stdout；它从不改你的文档。

这就是它全部的暴露面——后台没有任何别的东西在跑。
...
**B 层要薄。** B 层的一条断言，是指向其 A 层制品的*聚合 + 指针*——绝不是把那篇制品的叙述再抄一遍。逐 session 的细节沉淀进 A（允许无限增长）；B 靠指针、不靠复制，保持小巧。A 层体量大是预期内的，只有 B 层增长才算膨胀——这也是漂移雷达的膨胀检测*只*扫 B 的原因。
...
- **Phase 1 —— 沉淀。** 读取你的 A/B 地图，观察你的文档结构，产出一份清单：这次 session 的发现该落到哪里——分类为 A（新的不可变制品）或 B（SSOT 更新），每条都附一段具体的 diff 草图。如果 A/B 槽位是空的，它只问**一次**——填进去（推荐）、仅本次使用建议、或直接跳过。要应用、部分应用、全跳过、还是把这次标记为探索型 session，都由你定。没有你的点头，什么都不会被写入。它把 B 层的改动保持得很薄——一条聚合加一个指针——并把逐 session 的叙述导进 A（或交接文档），于是你的 SSOT 不会一个 session 一个 session 地膨胀。
...
- ✅ A/B 的薄化推论：B 是「聚合 + 指针」层，逐 session 叙述归 A
- ✅ 通用的漂移检测（失效链接、过期时间戳、孤儿文档、重复 H1），外加 A/B 感知的 B 层膨胀检测
